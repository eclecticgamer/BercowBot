...
class BotClient(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        if not set(message_words).isdisjoint(set(politics_triggers)) and message.channel.id not in preferences["politics_channels"]:

            response_options = []

            response_options.append('ORRRRDEEEEEERRRRRRRRRRRR! I cannot allow political discussion in this forum. Please respect other members and discuss these issues in the correct place.')
            response_options.append('Order! I don\'t wish to hear it; neither does the House. The Right Honourable Member must take their political discussions to the correct forum.')
            response_options.append('Mr {0.mention}! You really are a rather over-excited individual! You need to write out 1,000 times \'I will solely discuss politics in the correct channel.\''.format(message.author))
...
bot = BotClient(command_prefix=command_prefix)
